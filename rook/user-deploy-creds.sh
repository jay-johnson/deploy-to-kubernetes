...
# usage: user-deploy-creds.sh <optional - username> <optional - name for files like default-user>
...
if [[ "${object_store}" == "ceph" ]]; then
    kubectl exec -n rook-ceph -it rook-ceph-tools -- /bin/bash -c "radosgw-admin user info --uid=${username}" > ${json_file}
    ...
    internal_cluster_host="s3-storage.rook-minio"
    ...
    good "applying secret namespace=rook-minio-system rook.s3.user from secret file ${target_file_path}"
    kubectl apply -n rook-minio-system -f ${target_file_path}
    inf ""
