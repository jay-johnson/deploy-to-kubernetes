...
if [[ -e ${path_to_env_file} ]]; then
    ...
    if [[ "${debug}" == "1" ]]; then
        echo "kubectl exec -n rook-ceph -it rook-ceph-tools -- /bin/bash -c '. /creds && s3cmd ls --no-ssl --host=\${AWS_HOST}"
    fi
    kubectl exec -n rook-ceph -it rook-ceph-tools -- /bin/bash -c "echo 'starting' && . /creds && s3cmd ls --no-ssl --host=\${AWS_HOST}"
    if [[ "${debug}" == "1" ]]; then
        inf ""
    fi
else
